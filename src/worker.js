...
async function handleAPI(request, url, env) {
  ...
  try {
    ...
    if (url.pathname === '/api/exception' && request.method === 'POST') {
      ...
      if (excepted) {
        const countRes = await db.prepare('SELECT COUNT(*) as cnt FROM user_exceptions WHERE user_id = ? AND category = ?').bind(user_id, category).first();
        if (countRes.cnt >= MAX_EXCEPTIONS_PER_CAT) {
          return jsonRes({ error: 'limit_reached', message: 'Limite atteinte (' + MAX_EXCEPTIONS_PER_CAT + ' max). Demande \u00e0 ton parrain.' }, 403);
        }
...
function renderExcDomains(){
  ...
  list.innerHTML=filtered.map(d=>'<div class="exc-item"><span class="domain '+(d.excepted?'excepted':'')+'">'+d.domain+'</span><label class="tg"><input type="checkbox" '+(d.excepted?'checked':'')+' onchange="toggleException(\\''+d.domain+'\\',this.checked)" /><span class="tg-s"></span></label></div>').join('');
}
...
async function toggleException(domain,excepted){
  ...
  try{
    ...
    if(data.error==='limit_reached'){
      hideLoader();
      var pin=prompt('Ajouter une exception n\u00e9cessite le code secret de ton parrain :');
      if(!pin)return;
      showLoader();
      var pRes=await fetch(API+'/parrain/exception',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({parrain_id:state.user.parrain_id,pin:pin,user_id:state.user.id,category:excCategory,domain:domain,excepted:excepted})});
      var pData=await pRes.json();
      if(pData.error){hideLoader();alert(pData.error);return;}
    } else if(data.error){hideLoader();alert(data.error);return;}
    const d=excDomains.find(x=>x.domain===domain);if(d)d.excepted=excepted;renderExcDomains();
